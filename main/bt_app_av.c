...
void bt_i2s_driver_install(void)
{
    ...
    dac_continuous_config_t cont_cfg = {
        ...
        .offset = 127,
        .clk_src =
            DAC_DIGI_CLK_SRC_DEFAULT, // Using APLL as clock source to get a wider frequency range
...
static void volume_set_by_controller(uint8_t volume)
{
    ESP_LOGI(BT_RC_TG_TAG, "Volume is set by remote controller to: %" PRIu32 "%%",
...
static void bt_av_hdl_a2d_evt(uint16_t event, void *p_param)
{
    ...
    switch (event)
    {
        ...
        case ESP_A2D_AUDIO_CFG_EVT:
        {
            ...
            if (p_mcc->type == ESP_A2D_MCT_SBC)
            {
                ...
                dac_continuous_config_t cont_cfg = {
                    ...
                    .freq_hz = sample_rate,
                    .offset = 127,
                    .clk_src = DAC_DIGI_CLK_SRC_DEFAULT, // Using APLL as clock source to get a
                ...
                ESP_LOGI(BT_AV_TAG, "Configure audio player: 0x%x-0x%x-0x%x-0x%x-0x%x-%d-%d",
                         p_mcc->cie.sbc_info.samp_freq, p_mcc->cie.sbc_info.ch_mode,
                         p_mcc->cie.sbc_info.block_len, p_mcc->cie.sbc_info.num_subbands,
        ...
        case ESP_A2D_SEP_REG_STATE_EVT:
        {
            ...
            {
                ESP_LOGI(BT_AV_TAG, "A2DP register SEP success, seid: %d",
                         a2d->a2d_sep_reg_stat.seid);
            }
            else
            {
                ESP_LOGI(BT_AV_TAG, "A2DP register SEP fail, seid: %d, state: %d",
        ...
        case ESP_A2D_SNK_SET_DELAY_VALUE_EVT:
        {
            ...
            {
                ESP_LOGI(BT_AV_TAG, "Set delay report value: success, delay_value: %u * 1/10 ms",
...
static void bt_av_hdl_avrc_ct_evt(uint16_t event, void *p_param)
{
    ...
    switch (event)
    {
        ...
        {
            ESP_LOGI(BT_RC_CT_TAG, "AVRC passthrough rsp: key_code 0x%x, key_state %d, rsp_code %d",
        ...
        {
            ESP_LOGI(BT_RC_CT_TAG, "AVRC metadata rsp: attribute id 0x%x, %s", rc->meta_rsp.attr_id,
                     rc->meta_rsp.attr_text);
#if CONFIG_EXAMPLE_AVRCP_CT_COVER_ART_ENABLE
            if (rc->meta_rsp.attr_id == 0x80 && cover_art_connected && cover_art_getting == false)
            {
                /* check image handle is valid and different with last one, wo dont want to get an
                 * image repeatedly */
        ...
        case ESP_AVRC_CT_COVER_ART_STATE_EVT:
        {
            ...
            else
            {
                ...
                ESP_LOGW(BT_RC_CT_TAG, "Cover Art Client disconnected, reason:%d",
                         rc->cover_art_state.reason);
            }
#endif
            break;
        }
        ...
        {
#if CONFIG_EXAMPLE_AVRCP_CT_COVER_ART_ENABLE
            /* when rc->cover_art_data.final is true, it means we have received the entire image or
...
static void bt_av_hdl_avrc_tg_evt(uint16_t event, void *p_param)
{
    ...
    switch (event)
    {
        ...
        {
            ESP_LOGI(BT_RC_TG_TAG, "AVRC passthrough cmd: key_code 0x%x, key_state %d",
        ...
        {
            ESP_LOGI(BT_RC_TG_TAG, "AVRC register event notification: %d, param: 0x%" PRIx32,
        ...
        {
            ESP_LOGI(BT_RC_TG_TAG, "AVRC remote features: %" PRIx32 ", CT features: %x",
...
{
    ESP_LOGI(BT_AV_TAG, "data_len: %d, number_frame: %d, ts: %lu", audio_buf->data_len,
             audio_buf->number_frame, audio_buf->timestamp);
...
{
#if CONFIG_EXAMPLE_AVRCP_CT_COVER_ART_ENABLE
    /* we must handle ESP_AVRC_CT_COVER_ART_DATA_EVT in this callback, copy image data to other buff
     * before return if need */
    if (event == ESP_AVRC_CT_COVER_ART_DATA_EVT &&
